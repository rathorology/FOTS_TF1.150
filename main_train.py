...
tf.app.flags.DEFINE_string('pretrained_model_path', 'SynthText_6_epochs', '')
tf.app.flags.DEFINE_integer('train_stage', 2,
...
def main(argv=None):
    ...
    f_score, f_geometry, recognition_logits = build_graph(input_images, input_transform_matrix, input_box_masks,
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % 10 == 0:
                ...
                start = time.time()
                print(
                    'Step {:06d}, detect_loss {:.4f}, recognize_loss {:.4f}, total loss {:.4f}, {:.2f} seconds/step, {:.2f} examples/second'.format(
            ...
            if step % FLAGS.save_summary_steps == 0:
                ...
                _, tl, summary_str = sess.run([train_op, total_loss, summary_op], feed_dict={input_images: data[0],
                                                                                             ...
                                                                                             input_training_masks: data[4]})
                """
                dl, rl, tl, _, summary_str = sess.run([d_loss, r_loss, total_loss, train_op, summary_op],
                                                      feed_dict=inp_dict)
