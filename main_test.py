                ...
                try:
                    ...
                    if boxes is not None and boxes.shape[0] != 0:
                        ...
                        for batch_index in range(input_roi_boxes.shape[0] // 32 + 1):  # test roi batch size is 32
                            ...
                            # Run end to end
                            recog_decode = sess.run(dense_decode, feed_dict={input_feature_map: shared_feature_map,
                    ...
                    """
                                ...
                                cv2.fillPoly(im[:, :, ::-1], [text_area.astype(np.int32).reshape((-1, 1, 2))],
                                             color=(255, 255, 0))
                                im_txt = cv2.putText(im[:, :, ::-1], recognition_result, (box[0, 0], box[0, 1]), font,
                                                     0.5,
                                                     (0, 0, 255), 1)
                    else:
